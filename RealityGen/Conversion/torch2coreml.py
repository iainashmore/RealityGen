...
def _convert_to_coreml(submodule_name, torchscript_module, sample_inputs,
    ...
    if os.path.exists(out_path):
        ...
        outputShape = ct.EnumeratedShapes(shapes=[[1, 3, 64, 64],[1, 3, 512, 512]],
                                          default=[1, 3, 64, 64])
                                          
        if submodule_name == "vae_decoder":
...
def convert_vae_decoder(pipe, args):
    ...
    class VAEDecoder(nn.Module):
        ...
        def forward(self, z):
            ...
            print(x.size())
            #x = x.unsqueeze(0)  # torch.Size([1, 3, 252, 252])
            out = torch._C._nn.adaptive_avg_pool2d(x,(64, 64))
            print("interpolate...")
            print(out.size())
            return out
...
#    new_layer = coreml_vae_decoder._spec.neuralNetwork.layers.add()
#    new_layer.name ="My new layer"
#    new_layer.activation.linear.alpha = 1
#    new_layer.input.append("image")
#    new_layer.output.append("new_output")

    # Set model metadata
    coreml_vae_decoder.author = f"Iain Ashmore"
    if args.xl_version:
    ...
    coreml_vae_decoder.output_description[
        "image"] = "Iain Generated image normalized to range [-1, 1]"
